...
#components
file_url <- paste0("https://www.bankofgreece.gr/Publications/financialstat", str_remove(str_sub(round_date(as_date(nearest_date), unit = "month"), end = 7), pattern = "-"), "_en.pdf")
...
bal_sheet <- bal_sheet %>%
  ...
  mutate(amount = str_trim(str_extract(str_remove_all(item, ","), " [0-9]+")), item = str_trim(str_remove_all(item, "[0-9]|,")))
